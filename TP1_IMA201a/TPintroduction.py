...
def view_spectre(im,option=2,hamming=False):
...
# histogrames
im_flottant1=np.float32(skio.imread("images/vue1.tif")) #gris
im_flottant2-np.float32(skio.imread("images/fleur.tif")) #couleur
im_sombre=skio.imread("images/sombre.jpg")
im_sombre=im_sombre.mean(axis=2)

plt.hist(im_flottant1.reshape((-1,)) , bins=255) # obtenir l'histograme
plt.hist(im_sombre.reshape((-1,)), bins=255)

rouge=im_flottant2[:,:,0] #extraire le canal rouge d'une image couleur
plt.hist(rouge. reshape((-1, )), bins=255)

moyenne=im_flottant2,mean(axis-2) #moyennant les canaux couteurs
plt.hist(moyenne.reshape((-1,)),bins-255)

# histogramme cumule
(histo,bins)=np.histogram(im_sombre.reshape((-1,)),np.arange(0,256))#cumulé
histo=histo/histo.sum()
histocum=histo.cumsum()
plt.plot(histocum) # plot histogran cumule

imequal = histocum[np.uint8(im_sombre)]
plt.hist(imequal.reshape((-1,)), bins=255)

img_eq = exposure.equalize_hist(imequal)
plt.hist(img_eq.reshape((-1,)), bins=255)
(histo,bins)=np.histogram(imequal.reshape((-1,)),np.arange(0,256))#cumulé
histo=histo/histo.sum()
histocum=histo.cumsum()
plt.plot(histocum)

imbr=noise(im_flottant1, 10)
viewimage_color(imbr,normalise=False)

#effet sur histogramme
plt.hist(im_flottant1.reshape((-1,)), 255)
plt.show()
...
imequal=exposure.equalize_hist(im)
viewimage(imequal)
plt.hist(imequal.reshape((-1,)) , bins=255)
(histo,bins)=np.histogram(imequal*255,np.arange(0,256)) #le reshape en inutile pour np.histogram, mais on le laisse pour la compatibilite avec plt.hist
histo=histo/histo.sum()
histocum=histo.cumsum()
plt.plot(histocum)
...
image_equal=exposure.equalize_hist(u) #egalisation
viewimage(image_equal)
plt.hist(image_equal.reshape((-1,)) , bins=255)
(histo,bins)=np.histogram(image_equal*255,np.arange(0,256)) #le reshape en inutile pour np.histogram, mais on le laisse pour la compatibilite avec plt.hist
histo=histo/histo.sum()
histocum=histo.cumsum()
plt.plot(histocum)
...
viewimage(seuil(im,70))

viewimage(seuil(noise(im,40),128)) #exemple de dithering
plt.hist(seuil(noise(im,40),128).reshape((-1,)) , bins=255)
(histo,bins)=np.histogram(seuil(noise(im,40),128)*255,np.arange(0,256)) #le reshape en inutile pour np.histogram, mais on le laisse pour la compatibilite avec plt.hist
histo=histo/histo.sum()
histocum=histo.cumsum()
plt.plot(histocum)
...
gr=gradx(im)
plt.hist(gr.reshape((-1,)) , bins=255)
plt.plot(np.log(np.histogram(gradx(gr),255)[0]))
...
im2=skio.imread('images/masque_gauss_centre.tif')
viewimage(im)
viewimage(im2)
#%% FIN TP INTRODUCTION
