...
# une rotation 45 degres
im=skio.imread('images/bat.tif')
viewimage(im)
rot_ppv=rotation(im, theta=45,alpha=1.0,x0=None,y0=None,ech=0,clip=True)
viewimage(rot_ppv)
rot_bil=rotation(im, theta=45,alpha=1.0,x0=None,y0=None,ech=1,clip=True)
viewimage(rot_bil)

# 8 rotations de 45 degrees
im=skio.imread('images/bat.tif')
viewimage(im)

for i in range(0,7):
    rot_ppv=rotation(rot_ppv, theta=45,alpha=1.0,x0=None,y0=None,ech=0,clip=True)
    rot_bil=rotation(rot_bil, theta=45,alpha=1.0,x0=None,y0=None,ech=1,clip=True)

viewimage(rot_ppv)
viewimage(rot_bil)    

# 1 rotation avec zoom 1/2
rot_ppv=rotation(im, theta=45,alpha=0.5,x0=None,y0=None,ech=0,clip=True)
rot_bil=rotation(im, theta=45,alpha=0.5,x0=None,y0=None,ech=1,clip=True)
viewimage(rot_ppv)
viewimage(rot_bil)   

# FILTRAGE LINEAIRE ET MEDIAN
im=skio.imread('images/dice5.tif')

im_filtre1=filtre_lineaire(im, get_gau_ker(5))
im_filtre2=filtre_lineaire(im, get_gau_ker(10))
taille_matrix1=get_gau_ker(5).shape  # ver tamanho da matrix
taille_matrix1=get_gau_ker(10).shape
viewimage(im)
viewimage(im_filtre1)
viewimage(im_filtre2)

im_fm1=median_filter(im,typ=1,r=1,xy=None)
im_fm2=median_filter(im,typ=1,r=10,xy=None)
viewimage(im_fm1)
viewimage(im_fm2)

im=skio.imread('images/pyramide.tif')
im_noise=noise(im,10)
viewimage(im)
viewimage(im_noise)
im_n_fl=filtre_lineaire(im_noise, get_gau_ker(5))
viewimage(im_n_fl)
var_image(im_n_fl,20,70,56,116) #var image filtrée
var_image(im_noise,20,70,56,116) #var image bruitée

im=skio.imread('images/angiogr.tif')
im_noise=noise(im,10)
im_n_fl=filtre_lineaire(im_noise, get_gau_ker(5))
im_n_fm=median_filter(im_noise,typ=1,r=3,xy=None)
viewimage(im)
viewimage(im_noise)
viewimage(im_n_fl)
viewimage(im_n_fm)
...
im_fl=filtre_lineaire(im, get_cst_ker(5))
im_fm=median_filter(im,typ=1,r=3,xy=None)
viewimage(im)
viewimage(im_fl)
viewimage(im_fm)

im=skio.imread('images/carre_orig.tif')
im_fl=filtre_lineaire(im, get_cst_ker(3))
im_fm=median_filter(im,typ=1,r=3,xy=None)
viewimage(im)
viewimage(im_fl)
viewimage(im_fm)
...
im=skio.imread('images/brain.tif')
im_fl=filtre_lineaire(im, get_gau_ker(3))
im_fi=filtre_inverse(im_fl, get_gau_ker(3))
viewimage(im)
viewimage(im_fl)
viewimage(im_fi)
...
im=skio.imread('images/brain.tif')
im_fl=filtre_lineaire(im, get_gau_ker(3))
im_fn=noise(im_fl,10)
im_fi=filtre_inverse(im_fn, get_gau_ker(3))
viewimage(im_fn)
viewimage(im_fi)

# determiner le noyau de convolution
im=skio.imread('images/carre_flou.tif')
im_or=skio.imread('images/carre_orig.tif')
im_fl=filtre_lineaire(im_or, get_cst_ker(0.5))
viewimage(im)
viewimage(im_or)
viewimage(im_fl)
...
im=skio.imread('images/carre_flou.tif')
im_noise=noise(im,5)
im_wiener=wiener(im_noise,get_cst_ker(3),100)
viewimage(im)
viewimage(im_noise)
viewimage(im_wiener)

im_wiener=wiener(im_noise,get_cst_ker(3),10)
viewimage(im_wiener)

# applications - 1
im=skio.imread('images/carre_orig.tif')
im_br=noise(im,5)
im_fm=median_filter(im_br,typ=2,r=4,xy=None) #FM circulaire rayon=4
viewimage(im)
viewimage(im_br)
viewimage(im_fm)

var_im_fm=var_image(im_fm, 0, 0, 2, 2) # variation d'un pixel dans l'image
for i in np.arange(0,100): # matrix de taille 1 a 100 avec un intervalle intier
    im_fl=filtre_lineaire(im_br, get_cst_ker(i))
    var_im_fl=var_image(im_fl, 0, 0, 2, 2) # meme pixel
    if(np.around(var_im_fm, 2) == np.around(var_im_fl,2)):
        print(i) # trouver le taille du noyau (taille de la matrix)
        
# applications - 2
im=skio.imread('images/carre_orig.tif')
im_br=noise(im,5)
im_wiener=wiener(im_br,get_cst_ker(3),10) # restauration wiener masque const taille=3

#modification de la fonction wiener:
def wiener_mod(im,K):
    """effectue un filtrage de wiener de l'image im par le filtre K.
       lamb=0 donne le filtre inverse
    ...
    #densite spectrale
    sigma_w=g**2
    sigma=im.var()*im.size
    #fonction de mutilplication
    mul=np.conj(k)/(abs(k)**2+(sigma/sigma_w))
    #filtrage de wiener
    fout=g*mul
...
im_wiener_mod=wiener_mod(im_br,get_cst_ker(3))

viewimage(im)
viewimage(im_br)
viewimage(im_wiener)
viewimage(im_wiener_mod)
