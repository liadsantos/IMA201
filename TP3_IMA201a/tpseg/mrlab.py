...
def dericheGradX(ima,alpha):
    ...
    for i in range(nl):
        ...
        for j in range(2,nc):
            b1[j] = l[j-1] + 2*ae*b1[j-1] - ae*ae*b1[j-2]    # LIGNE MODIFIE
        b1[0]=b1[2]
        b1[1]=b1[2]
        
        for j in range(nc-3,-1,-1):
            b2[j] = l[j+1] + 2*ae*b2[j+1] - ae*ae*b2[j+2]    # LIGNE MODIFIE
        b2[nc-2]=b2[nc-3]
        b2[nc-1]=b2[nc-3]
